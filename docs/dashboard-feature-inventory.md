...
| Source fill workflow | Prioritized zero-call workflow from priced-in preflight evidence-plan steps, including dependencies, full-scan gap counts, stock-like market-bar gap in `bars status`, short TUI inspect commands, next source action, the all-source plan command, market-bar `after_market_bars_clear` preview, and a compact overview hint for coverage-first versus decision-shortcut work | `overview`, `ops`, `market-bars status`, `bars status` in TUI, `dashboard-snapshot --page ops --json`, `batch all` in TUI | Know the next data layer to inspect before running provider chunks; keep current action distinct from the response expected after the blocker clears. |
