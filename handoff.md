...
- The active goal remains unchanged: MarketRadar should scan the broad stock market and identify stocks where market emotion or expectations have not yet been matched by price.
- This slice does not call Polygon/Massive, SEC, Schwab, OpenAI, broker, order, web, shell, or filesystem tools from the app.
- This is useful because the dashboard command response is where a human operator asks for the full source map. It should distinguish total market-wide gaps from rows that are plannable now, routed elsewhere, or truly blocked.

Fix in this slice:

- TUI `batch all` source rows now print `gaps=`, `plan=`, `routed=`, and `blocked=` instead of only total gaps and batch counts.
- The displayed `blocked=` count is the remaining blocked rows after routed rows, using diagnostic blocked rows when present and otherwise `unplannable - routed`.
- CLI mission-roadmap text now uses the same blocked-leftover display rule.
- README documents that the terminal dashboard `batch all` response mirrors the on-demand gap summary without slowing every TUI frame.

Validation observed in this slice:

- `C:\Users\fpan1\MarketRadar\.venv\Scripts\python.exe -m pytest tests\integration\test_dashboard_demo_seed_cli.py::test_dashboard_batch_command_opens_full_scan_source_batch_plan tests\integration\test_dashboard_demo_seed_cli.py::test_dashboard_batch_all_response_separates_plan_route_and_blocked_rows -q` passed.
- `C:\Users\fpan1\MarketRadar\.venv\Scripts\python.exe -m ruff check src\catalyst_radar\dashboard\tui.py src\catalyst_radar\cli.py tests\integration\test_dashboard_demo_seed_cli.py` passed with `All checks passed!`.
- `C:\Users\fpan1\MarketRadar\.venv\Scripts\python.exe -m py_compile src\catalyst_radar\dashboard\tui.py src\catalyst_radar\cli.py tests\integration\test_dashboard_demo_seed_cli.py` exited 0.
- `git diff --check` exited 0.
- Live zero-call CLI smoke against `schwab-live.db`, with `PYTHONPATH` pinned to this worktree, printed catalyst-events `gaps=12075 plan=5510 routed=6563 blocked=2 calls=5` and `external_calls_made` stayed 0.
- Live zero-call TUI command smoke for `_apply_command('batch all')` against `schwab-live.db` printed `market_bars=attention gaps=523 plan=0 blocked=523` and `catalyst_events=ready gaps=12075 plan=5510 routed=6563 blocked=2`.

Current live blocker:

- The full-market priced-in answer remains blocked by 523 missing market bars for 2026-05-15.
- The manual CSV still has 0 complete rows and the saved grouped-daily JSON is still missing.
- This slice improves human dashboard/CLI interpretation only; it does not capture, import, or reduce the universe.
...
## Latest TUI Manual Market-Bar Commands

Goal alignment / drift check:

- The active goal remains: MarketRadar should scan the broad stock market and identify stocks where market emotion or expectations have not yet been matched by price.
- The current first useful blocker is still `market_bars`; no provider calls were made while implementing this slice.
- The saved-file path was available in the TUI, but the zero-call manual CSV path still required leaving the dashboard.
- This slice keeps the dashboard useful as a replacement operator UI: it can generate, preview, and execute the manual market-bar repair workflow directly with clear provider/write boundaries.

Fix in this slice:

- Added TUI commands on the Run page:
  - `bars manual template` generates the missing-bar CSV for stock-like active rows by default.
  - `bars manual full template` generates the full active-universe missing-bar CSV when the operator intentionally asks for full scope.
  - `bars manual import` previews complete rows only, makes 0 provider calls, and makes 0 DB writes.
  - `bars manual import execute` imports only completed rows into the local DB and makes 0 provider calls.
- The manual TUI path reuses `write_manual_market_bars_template(...)` and `import_manual_market_bars(...)` instead of creating a parallel ingestion path.
- Static and Textual help now list both manual and saved market-bar commands.
- README now documents the TUI manual commands next to the repair-plan workflow.

Validation observed in this slice:

```powershell
C:\Users\fpan1\MarketRadar\.venv\Scripts\python.exe -m ruff check src\catalyst_radar\dashboard\tui.py tests\integration\test_dashboard_demo_seed_cli.py
$env:PYTHONPATH='C:\Users\fpan1\MarketRadar\.worktrees\tui-manual-market-bars\src'; C:\Users\fpan1\MarketRadar\.venv\Scripts\python.exe -m pytest tests\integration\test_dashboard_demo_seed_cli.py::test_dashboard_bars_manual_template_and_import_are_zero_call_actions tests\integration\test_dashboard_demo_seed_cli.py::test_dashboard_bars_manual_full_scope_uses_full_template_path tests\integration\test_dashboard_demo_seed_cli.py::test_dashboard_bars_saved_validate_and_import_fixture_are_operator_actions -q
git diff --check
$env:PYTHONPATH='C:\Users\fpan1\MarketRadar\.worktrees\tui-manual-market-bars\src'; C:\Users\fpan1\MarketRadar\.venv\Scripts\python.exe -m catalyst_radar.cli dashboard-tui --once --page help | Select-String 'bars manual|bars saved'
$env:PYTHONPATH='C:\Users\fpan1\MarketRadar\.worktrees\tui-manual-market-bars\src'; C:\Users\fpan1\MarketRadar\.venv\Scripts\python.exe -m catalyst_radar.cli dashboard-tui --once --page run | Select-String 'External calls made|Manual market-bar|bars manual'
```

Observed results: Ruff passed, focused pytest passed 3 tests, `git diff --check` passed, the static help listed both manual and saved market-bar commands, and the run-page smoke still reported `External calls made: 0`.
...
- The active goal remains: scan the broad market and tell whether any stock's
  price has not yet matched market emotion/expectations.
- The goal explicitly has two operating surfaces: CLI/API and dashboard. The
  previous slice added a safe CLI preview for saved Polygon/Massive grouped
  daily files, but the API still lacked the same no-write operation.
- This slice keeps the same product blocker in view: full-market priced-in
  analysis is still blocked by scan-date market bars. It does not change the
  scoring model, scan eligibility, dashboard layout, live provider execution,
  agent behavior, Schwab behavior, or broker/order behavior.
- This slice makes 0 Polygon/Massive, SEC, Schwab, OpenAI, broker, order, or
  provider calls.

Fix in this slice:

- Extracted the saved grouped-daily fixture preview into
  `catalyst_radar.connectors.polygon_fixture.preview_polygon_grouped_daily_fixture`.
- The CLI `ingest-polygon grouped-daily --fixture ... --validate-only` now uses
  that shared helper.
- Added API request model `MarketBarsProviderFixturePreviewRequest`.
- Added API endpoint:
  `POST /api/radar/market-bars/provider-fixture-preview`
- The endpoint:
  - requires analyst role like the other market-bar write-adjacent endpoints;
  - reads a saved grouped-daily JSON fixture from disk;
  - returns the same `polygon-grouped-daily-fixture-preview-v1` payload as the
    CLI preview;
  - reports coverage against active and stock-like local securities;
  - reports `external_calls_made=0` and `db_writes_made=0`;
  - does not save provider health, job runs, raw provider records, normalized
    provider records, incidents, or daily bars;
  - returns HTTP 422 for invalid/missing local fixture files.
- Repair-plan and priced-in provider-fill payloads now expose:
  - `provider_saved_file_validate_command`
...
```powershell
.\.venv\Scripts\python.exe -m ruff check src\catalyst_radar\connectors\polygon_fixture.py src\catalyst_radar\cli.py src\catalyst_radar\api\routes\radar.py src\catalyst_radar\market\manual_bars.py src\catalyst_radar\dashboard\data.py tests\integration\test_api_routes.py tests\integration\test_provider_ingest_cli.py tests\integration\test_dashboard_data.py tests\integration\test_security_boundaries.py
.\.venv\Scripts\python.exe -m pytest tests\integration\test_api_routes.py::test_post_radar_market_bars_provider_fixture_preview_is_zero_write tests\integration\test_api_routes.py::test_post_radar_market_bars_provider_fixture_preview_rejects_missing_file tests\integration\test_polygon_ingest_cli.py::test_polygon_grouped_daily_fixture_validate_only_makes_no_writes tests\integration\test_provider_ingest_cli.py::test_market_bars_repair_plan_reports_manual_and_guarded_provider_paths tests\integration\test_dashboard_data.py::test_priced_in_full_scan_audit_warns_for_stale_eod_provider_health -q
.\.venv\Scripts\python.exe -m pytest tests\integration\test_security_boundaries.py -q
.\.venv\Scripts\python.exe -m pytest tests\integration\test_api_routes.py::test_post_radar_market_bars_template_and_import_use_database_universe tests\integration\test_api_routes.py::test_post_radar_market_bars_import_complete_rows_only_is_incremental tests\integration\test_api_routes.py::test_post_radar_market_bars_template_and_import_can_scope_to_stocks tests\integration\test_api_routes.py::test_post_radar_market_bars_provider_fixture_preview_is_zero_write tests\integration\test_api_routes.py::test_post_radar_market_bars_provider_fixture_preview_rejects_missing_file -q
...
- Ruff passed.
- Focused API/CLI/provider/dashboard regression tests passed: `5 passed`.
- Security-boundary tests passed: `13 passed`.
- Market-bar API focused tests passed: `5 passed`.
- CLI validate-only JSON smoke returned schema
  `polygon-grouped-daily-fixture-preview-v1`, `status=ready_with_rejections`,
  `external_calls_made=0`, and `db_writes_made=0`.
- `git diff --check` passed.

Next useful product action:

- Clear the `market_bars` blocker. The safe API path is now:
  `POST /api/radar/market-bars/provider-fixture-preview` with
  `expected_as_of=2026-05-15` and
  `fixture_path=data\local\polygon-grouped-daily-2026-05-15.json`.
- If the preview covers the missing bars, use the CLI saved-file import path
  already shown by the repair plan, then rerun quick status and the priced-in
  audit.
