...
def _print_shadow_mode_run(payload: Mapping[str, object]) -> None:
    ...
    print(
        ...
        f"provider_calls_planned={payload.get('provider_calls_planned') or 0} "
        f"provider_calls_made={payload.get('provider_calls_made') or 0} "
        f"db_writes_planned={payload.get('db_writes_planned') or 0} "
...
def _llm_client_for_provider(*, config: AppConfig, fake: bool):
    ...
    if is_premium_llm_provider(provider):
