...
def _ingest_polygon_provider(
    *,
    ...
    try:
        if validate_only and (polygon_command != "grouped-daily" or fixture_path is None):
            print(
                "polygon fixture validation requires grouped-daily --fixture",
                file=sys.stderr,
            )
            return 2
        ...
        if validate_only:
            payload = _polygon_grouped_daily_fixture_preview(
                connector=connector,
                request=request,
                market_repo=market_repo,
                date_value=date_value,
    ...
    except (FileNotFoundError, json.JSONDecodeError, ProviderIngestError, ValueError) as exc:
        print(f"polygon ingest failed: {exc}", file=sys.stderr)
        return 1

    if json_output:
        print(json.dumps(_provider_result_payload(result), sort_keys=True))
    else:
        _print_provider_result(result)
    return 0


def _polygon_grouped_daily_fixture_preview(
    *,
    ...
    if date_value is None or fixture_path is None:
        msg = "fixture preview requires date and fixture path"
...
def _print_manual_market_bars_repair_plan(payload: Mapping[str, object]) -> None:
    ...
    if saved_file_command:
        ...
        if saved_file_validate_command:
            print(
                "provider_saved_file_validate="
                f"external_calls={payload.get('provider_saved_file_external_call_count')} "
                f"command={_compact_cli_text(saved_file_validate_command)}"
            )
        print(
        ...
        if isinstance(provider_plan, Mapping) and provider_plan:
            ...
            if saved_file_command:
                ...
                if saved_file_validate_command:
                    print(
                        "    saved_file_validate="
                        f"{_compact_cli_text(saved_file_validate_command)}"
                    )
                print(
                    "    saved_file_import="
...
def _provider_result_payload(result: ProviderIngestResult) -> dict[str, object]:
    return {
...
def _print_polygon_grouped_daily_fixture_preview(
    ...
    print(
        "polygon_grouped_daily_fixture_preview "
        f"status={payload.get('status')} "
        f"date={payload.get('date')} "
        f"path={payload.get('fixture_path')} "
        f"raw={payload.get('raw_count')} "
        f"normalized={payload.get('normalized_count')} "
        f"daily_bars={payload.get('daily_bar_count')} "
        f"rejected={payload.get('rejected_count')} "
        f"abort_rejections={payload.get('abort_rejection_count')} "
        f"external_calls={payload.get('external_calls_made')} "
    ...
    print(
        "stock_like "
        f"active={coverage.get('stock_like_active_count', 0)} "
        f"existing={coverage.get('stock_like_existing_as_of_bar_count', 0)} "
        f"covers_missing={coverage.get('stock_like_covered_by_fixture_count', 0)} "
        f"missing_after={coverage.get('stock_like_missing_after_import_count', 0)}"
    )
    print(f"next_action={payload.get('next_action')}")
