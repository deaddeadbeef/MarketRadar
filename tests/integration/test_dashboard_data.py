...
def test_dashboard_snapshot_payload_exposes_trading_workbench_contract(
    ...
    assert trade_runbook["source_tool"] == "market-radar"
    assert trade_runbook["ticker"] == "MSFT"
    assert trade_runbook["decision_card_id"] == "card-msft-latest"
    assert trade_runbook["active_step_id"] == "decision-review"
    assert trade_runbook["primary_next_action"] == (
        "Review decision readiness before continuing."
    )
    assert trade_runbook["steps"] == [
        {
            "id": "decision-review",
            "rank": 1,
            "module": "review",
    ...
    assert trade_runbook["metrics"] == {
        "step_count": 8,
        "blocked_step_count": 2,
        "approval_required_count": 1,
        "disabled_step_count": 1,
        "preview_step_count": 2,
        "local_write_step_count": 1,
        "external_calls_made": 0,
    }
    assert trade_runbook["external_calls_made"] == 0
    assert trade_runbook["db_writes_made"] == 0
    assert trade_runbook["broker_order_submitted"] is False
...
def test_priced_in_full_scan_audit_payload_consolidates_current_state(
    ...
    assert payload["primary_scan"]["ranked_rows"] == 2
    assert payload["primary_scan"]["shortlist_role"] == (
        "priority_lens_not_scan_scope"
    )
    assert "Shortlists are priority lenses" in payload["primary_scan"][
        "scope_boundary"
    ]
    ...
    if payload["answer_shortlist"]["rows"]:
        ...
        assert payload["answer_shortlist"]["rows"][0]["drilldown"][
            "detail_command"
        ] == "catalyst-radar candidate-detail MSFT"
        assert payload["answer_shortlist"]["rows"][0]["drilldown"][
            "external_calls_made"
        ] == 0
...
def test_priced_in_answer_routes_zero_liquidity_residual_gap_to_review(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(tmp_path)
    ...
    assert detail["recommended_action"]["external_calls_required"] == 0
    assert payload["operator_next_step"]["action_kind"] == (
...
def test_live_activation_plan_rejects_sec_user_agent_placeholder() -> None:
    config = AppConfig(
        daily_market_provider="polygon",
        daily_provider="polygon",
        polygon_api_key="polygon-secret-value",
        daily_event_provider="sec",
        sec_enable_live=True,
        sec_user_agent="MarketRadar/0.1 your-email@example.com",
    )
    run_summary = {
        "step_count": 6,
        "required_step_count": 6,
