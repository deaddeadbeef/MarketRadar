...
def test_dashboard_empty_candidates_points_to_first_setup_blocker(
    ...
    database_url = f"sqlite:///{(tmp_path / 'empty-candidates.db').as_posix()}"
    monkeypatch.setenv("CATALYST_DATABASE_URL", database_url)
...
def test_dashboard_empty_agent_gate_points_to_first_setup_blocker(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
...
def test_dashboard_empty_themes_points_to_first_setup_blocker(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
    database_url = f"sqlite:///{(tmp_path / 'empty-themes.db').as_posix()}"
    monkeypatch.setenv("CATALYST_DATABASE_URL", database_url)

    assert main(["dashboard-tui", "--once", "--page", "themes"]) == 0
    output = capsys.readouterr()
    normalized = " ".join(output.out.split())
...
def test_alias_analysis_page_headers_are_human_labels() -> None:
    payload = _minimal_missing_real_results_payload()
    payload["themes"] = {"count": 0, "rows": []}
    payload["validation"] = {"report": {}, "latest_run": {}}
    payload["costs"] = {}
    payload["value_report"] = {}

    assert "Page: Themes" in render_dashboard_tui(payload, page="themes", width=120)
    assert "Page: Validation" in render_dashboard_tui(
        payload,
        page="validation",
        width=120,
    )
    assert "Page: Costs" in render_dashboard_tui(payload, page="costs", width=120)
...
def test_dashboard_ops_empty_state_names_empty_sections_and_setup_first(
    ...
    assert "First blocker            : Set up Active universe first" in output.out
    assert "Do first                 : Use the PowerShell command below" in output.out
    assert "Seed or refresh the universe with `catalyst-radar" not in output.out
...
def test_dashboard_setup_tutorial_starts_with_setup_command() -> None:
    ...
    assert results["1"] == "If you accept the call/write, run it in PowerShell."
    assert rows["2"] == "Press 2: Evidence Gaps"
...
def test_market_inbox_distinguishes_visible_page_from_full_queue() -> None:
    ...
    assert "NEXT SAFE ACTION: Open first Worth Reading row. Research only." in overview
    assert "Open 1 Worth Reading" not in overview
...
def test_ops_footer_prefers_workflow_coverage_first_over_research_rows() -> None:
    ...
    assert "NEXT SAFE ACTION: Market bars first: 2=command; Enter=plan." in ops
...
def test_tui_guides_market_bar_review_command_boundary():
    engine = create_engine("sqlite:///:memory:", future=True)
    config = AppConfig.from_env({})
    ...
    assert "No API calls/orders/writes." in update.message
    assert "PowerShell command, not a dashboard command" in update.message
    assert "market-bars residual-review --expected-as-of 2026-05-15" in update.message
    assert "Read-only market-bar review" in update.message
    assert "no provider, OpenAI, broker, order, or DB write calls" in update.message
    assert "Unknown command" not in update.message
...
def test_dashboard_run_page_next_safe_action_uses_operator_step():
    ...
    assert "Run the PowerShell command shown above." in action
    ...
    assert "0 provider calls, 0 DB writes; no approval." in action
    assert "run execute only if intended" not in action

    run = render_dashboard_tui(payload, page="run", width=120)
    normalized_run = " ".join(run.split())
    assert "NEXT SAFE ACTION: Run the PowerShell command shown above." in run
    ...
    assert "0 provider calls, 0 DB writes; no approval." in normalized_run
...
def test_modern_dashboard_tui_supports_mouse_navigation(
    ...
    async def run_app() -> None:
        async with app.run_test(size=(150, 44)) as pilot:
            ...
            assert "STOPLIGHT" in frame
            assert "First must fix:" in frame
            assert "0 calls, 0 orders" in frame
            ...
            assert "Evidence Gaps" in frame
            assert "Market bars first" in frame
            assert "2=command" in frame
            assert "Enter=plan" in frame
...
def test_modern_dashboard_inbox_names_exact_worth_reading_action(
    ...
    async def run_app() -> None:
        async with app.run_test(size=(150, 44)) as pilot:
            ...
            frame = html.unescape(app.export_screenshot()).replace("\xa0", " ")
            assert "Open first Worth Reading row. Research only." in frame
            assert "Open 1 Worth Reading" not in frame
            assert "Visible rows" not in frame
